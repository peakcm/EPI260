...
simulation <- cbind(simulation, dt)

#### PLOT OUTPUT ####

# SIMPLE R PLOT
plot(simulation$dt, simulation$S, type="l", col="blue", ylim=c(0,sum(inits)), xlab="Time", ylab="Number of individuals")
lines(simulation$dt, simulation$I, type="l", col="red")
lines(simulation$dt, simulation$R, type="l", col="darkgreen")
...
ggplot(data = simulation, aes(x = dt)) +
  geom_line(aes(y = S), color = "blue") +
  geom_line(aes(y = I), color = "red") +
  geom_line(aes(y = R), color = "darkgreen") +
...
ggvis(simulation, x = ~dt) %>%
  layer_lines(y = ~S, stroke = "blue") %>%
  layer_lines(y = ~I, stroke = "red") %>%
  layer_lines(y = ~R, stroke = "darkgreen") %>%
  add_axis("y", title = "Number of individuals") %>%
